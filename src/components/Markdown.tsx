...
export default function Markdown({ content }: { content: string }) {
    return (
        <ReactMarkdown
            components={{
                h1: ({ children }) => (
                    <h1 className='text-[16px] font-semibold mb-4 mt-3 pb-1 border-b-2 border-mac-ink'>{children}</h1>
                ),
                h2: ({ children }) => (
                    <h2 className='text-[14px] font-semibold mb-4 mt-3 pb-1 border-b border-mac-ink'>{children}</h2>
                ),
                ...
                hr: () => <hr className='border-t border-mac-ink my-4' />,
                img: ({ src, alt }) => (
                    <a
                        href={String(src)}
                        target='_blank'
                ...
                blockquote: ({ children }) => (
                    <blockquote className='border-l-2 border-mac-ink pl-2 italic my-4'>{children}</blockquote>
                ),
            }}
        >
            {content}
